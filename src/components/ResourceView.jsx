...
function ResourceView({
    ...
    const renderSlotItem = (item, indents) => {
        ...
        {indents}
                <button type="button" style={{cursor: 'pointer'}} className="slot-text txt-btn-dis"
                        onClick={() => slotClickedFunc(schedulerData, item)}>
        {resourceItemTemplateResolver ? resourceItemTemplateResolver(item, schedulerData) : item.slotName}
        </button>
      </span>
        ) : (
            <span className="slot-cell">
        {indents}
                <button type="button" className="slot-text txt-btn-dis"
        ...
        let slotItem = (
            <div title={item.slotTitle || item.slotName} className="overflow-text header2-text"
